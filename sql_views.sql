...
create view request_high_percent_errors as select to_char(log_date,'YYYY-MM-DD') as log_date, percent_errors from (select treads.log_date as log_date, total_reads, total_failures, (cast(total_failures as float)/cast(total_reads as float) * 100) as percent_errors from (select cast(time as date) as log_date, count(*) as total_reads from log where status = '200 OK'group by log_date) as treads
join (select cast(time as date) as log_date, count(*) as total_failures from log where status = '404 NOT FOUND' group by log_date) as tfails on treads.log_date = tfails.log_date) as report where percent_errors > 1;
